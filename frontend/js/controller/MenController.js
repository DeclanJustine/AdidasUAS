angular.module("myApp").controller("MenController", function ($scope, $window, $http) {
  ...
  const initializeProducts = async () => {
    try {
      ...
      $scope.originalProducts = allProducts.filter(product => 
        product.type === 'men' && product.category === 'originals'
      );
      
      $scope.sportsProducts = allProducts.filter(product => 
        product.type === 'men' && product.category === 'sports'
      );
      
      $scope.runningProducts = allProducts.filter(product => 
        product.type === 'men' && product.category === 'running'
      );
  ...
  $scope.searchProducts = function () {
    ...
    $scope.searchResults = allProducts.filter(product =>
      ...
      link: `#!/products/men/${product.category}/${product.id}`,
      subtitle: `Men's ${product.category.charAt(0).toUpperCase() + product.category.slice(1)}`,
      title: product.name,
